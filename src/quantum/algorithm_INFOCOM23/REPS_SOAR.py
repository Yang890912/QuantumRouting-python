...
class Path:
    ...
    def setTempNode(self, newTempNodeIndex):
        ...
        self.swapStatus[self.nodes[newTempNodeIndex]] = True
    def clearTemp(self):
...
class REPS_SOAR(AlgorithmBase):
    ...
    def trySwapped(self):
        for request in self.requests:
            ...
            swapped = False
            # print("SDpair = ", request.src.id, request.dst.id)
            for path in currentPath:
                # print("link", [link.entangled for link in path.links])
                # print("swap", [path.swapStatus[node] for node in path.nodes])
                        ...
                        if swapped == False:
                            ...
                            if link1.entangled and link2.entangled:
                                ...
                                if swapSuccess == False:
                                    ...
                                    Index = nodeIndex - 1
                                    while Index > path.tempNodeIndex + 1 and path.swapStatus[path.nodes[Index]] == True:
                                        Index -= 1
                                    left = Index + 1

                                    Index = nodeIndex + 1
                                    while Index < len(path.nodes) - 1 and path.swapStatus[path.nodes[Index]] == True:
                                        Index += 1
                                    right = Index - 1
    ...
    def forward(self):
        ...
        for request in self.requests:
            ...
            currentPath = request.currentPath()
            for path in currentPath:
                farestTrustedNodeIndex = -1
                src = request.src
                if self.k == 1:
                    next = path.nodes[path.tempNodeIndex + 1]
                    if path.swapStatus[next] == False and path.links[path.tempNodeIndex].entangled and next.remainingQubits >= 1 and next in self.topo.socialRelationship[src]:
                        farestTrustedNodeIndex = path.tempNodeIndex + 1

                for nodeIndex in range(path.tempNodeIndex + 1, len(path.nodes) - 1):
                    node = path.nodes[nodeIndex]
                    if path.swapStatus[node] == False:
                        if src in self.topo.socialRelationship[node] and node.remainingQubits >= 1:
                ...
                if forwardStep >= self.k:
                    ...
                    path.setTempNode(farestTrustedNodeIndex)
                    if path.nodes[farestTrustedNodeIndex] not in self.topo.socialRelationship[src]:
                        request.broken = True
        ...
        for request in finishedRequest:
            ...
            # self.totalNumOfTemporary += request.numOfTemp
